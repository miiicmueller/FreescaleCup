...
TerminalCallbackRecord MyTerminalCallbackRecords[] =
    {
	{
...
void TerminalCmd_S(char *arg)
	...
	{
	TERMINAL_PRINTF(
		"Invalid servo control string. There must be 2 command separated arguments between -100 and 100. Ex: S 43,-43");
	}
...
void TerminalCmd_H(char *arg)
	...
	TERMINAL_PRINTF(
		"Setting motor effort to to %d , %d\r\n", MotorSetting[0], MotorSetting[1]);

	TFC_SetMotorPWM((float) MotorSetting[0] / 100.0f,
		(float) MotorSetting[1] / 100.0f);

	}
    else
	{
	TERMINAL_PRINTF(
		"Invalid motor control string. There must be 2 command separated arguments between -100 and 100. Ex: H 43,-43\r\n");
	}
...
void TFC_ProcessTerminal()
	...
	case '\r':
		...
		while (TerminalLineBuf[i] <= 0x20
			&& (i < MAX_TERMINAL_LINE_CHARS))
		    {
		...
		for (j = 0; j < NUM_TERMINAL_COMMANDS; j++)
		    {
		    if (strcmp(TerminalCmdBuf,
			    MyTerminalCallbackRecords[j].CommandString) == 0)
			{
			TERMINAL_PRINTF("\r\n");
